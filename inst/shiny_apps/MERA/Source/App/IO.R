...
Update_Questionnaire<-function(MSClog){
  ...
  if(cond){
    ...
    }else if("nyears" %in% names(MSClog[[3]])){
      Lyear<<-Current_Year-2
      Syear<<-Current_Year-MSClog[[3]]$nyears+1  
      updateNumericInput(session, "Lyear",   value= Lyear)
        updateNumericInput(session, "Syear",   value= Lyear-MSClog[[3]]$nyears+1)
...
save_settings<-function(){
  ...
  settings[['catchcond']]<-input$catchcond
...
load_settings<-function(settings){
  ...
  if(!is.null(settings$catchcond))updateSelectInput(session,'Skin',selected=settings$catchcond)
